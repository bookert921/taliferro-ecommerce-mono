...
@Component({
  selector: 'app-result-upload-task',
  templateUrl: './result-upload-task.component.html',
  styleUrls: ['./result-upload-task.component.css']
})
export class ResultUploadTaskComponent implements OnInit, OnDestroy {
  ...
  startUpload() {
    try {
      const path = environment.RESULTS_PATH + `/${Date.now()}_${this.file?.name}`;
